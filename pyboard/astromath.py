...
class AstroMath:
    ...
    def JulianDate(self, year, month, day, utc=0):
        """
    ...
    def SiderialTime(self, year, month, day, utc=0):
        ...
        jd = self.JulianDate(year, month, day)
    ...
    def MakeMatrixSupplement(self, a, b, c):
    ...
    def Supplement(self, fixed, date):  # Here years, maybe should be JDays
        ...
        m = self.MakeMatrixSupplement(
            asec / SecondsPerRadian, bsec / SecondsPerRadian, csec / SecondsPerRadian
        )
        return m

    def Transform(self, ra, dec, matrix):  # returns  ra  & dec
    ...
    def CalcLST(self, year, month, day, ut):
        ...
        T0 = self.Map24(T0)
        gmst = self.Map24(T0 + ut * 1.002737909)
        lmst = 24.0 * self.frac((gmst - self.SiteLongitude_h / 15.0) / 24.0)
        return lmst
    ...
    def LSTNow(self):
        ...
        lst = self.CalcLST(year, month, day, ut)
        return lst

    def CalcJD(self, ny, nm, nd, ut):
        day = nd + ut / 24.0
        if nm == 1 or nm == 2:
            ny = ny - 1
            nm = nm + 12
    ...
    def JDNow(self):
        ...
        # Calculate floating point ut in hours
        ut = (milliseconds / 3600000) + (seconds / 3600) + (minutes / 60) + hours
        jd = self.CalcJD(year, month, day, ut)
        return jd

    # Precession from J2000 to EOD or back
    def Precession(self, ra_rad, dec_rad, dirflag):
        if dirflag > 0:
            return self.PrecessToEOD(2000.0, ra_rad, dec_rad)
        else:
            return self.PrecessToEpoch(2000.0, ra_rad, dec_rad)
    ...
    def PrecessToEOD(self, epoch, ra_rad, dec_rad):
        ...
        b = cos(ra_rad + zeta_rad) * cos(theta_rad) * cos(dec_rad) - sin(
            theta_rad
        ) * sin(dec_rad)
        c = cos(ra_rad + zeta_rad) * sin(theta_rad) * cos(dec_rad) + cos(
            theta_rad
        ) * sin(dec_rad)
        if c > 0.9:
    ...
    def PrecessToEpoch(self, epoch, ra_rad, dec_rad):
    ...
    def gms(self, grad_f):
        ...
        sekunden = (
            ((grad_f - int(grad_f)) * 60) - int((grad_f - int(grad_f)) * 60)
        ) * 60
    ...
    def HA_DEC_to_AZ_ALT(self, ha_rad, dec_rad, latitude_rad):
